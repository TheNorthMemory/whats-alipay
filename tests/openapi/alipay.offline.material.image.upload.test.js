...
describe('openapi/alipay.offline.material.image.upload', () => {
  ...
  let svcPrivateKey = readFileSync(join(__dirname, '../fixtures/mock-svc-pkcs1.key'))
  ...
  let mocks = function(requestUri, requestBody) {
    ...
    if (!Rsa.verify(payload, params.get('sign'), appPublicCert, params.get('sign_type'))) {
      data = `{"code":"40002","msg":"Invalid Arguments","sub_code":"isv.invalid-signature","sub_msg":"验签出错，建议检查签名字符串或签名私钥与应用公钥是否匹配，网关生成的验签字符串为：${payload.replace(/&(?!amp;)/g, '&amp;')}"}`
    } else {
  ...
  it('The request data signature on Invalid case, the gateway should responsed `40002:invalid-signature`', async () => {
    ...
    Decorator.client.defaults.transformRequest.unshift(function injectInvalidSign(data) {
