...
describe('openapi/alipay.trade.wap.pay', () => {
  ...
  beforeEach(() => {
    ...
    scope = nock('https://openapi.alipay.com', {
        filteringScope: scope => /^https:\/\/(?:openapi|tscenter|mclient).alipay.com/.test(scope),
      })
      .defaultReplyHeaders({
  ...
  it('Mock the browser\'s `form.submit()` event to the gateway URL then redirected to `mclient.alipay.com` for payment', async () => {
    scope.reply(302, mocks, {Location: 'https://mclient.alipay.com/cashier/mobilepay.htm?alipay_exterface_invoke_assign_target=invoke_blah32chars&alipay_exterface_invoke_assign_sign=_htt_e_m_r_pcytd%2F_encoded%2B_params%3D%3D'})
