...
describe('lib/formatter', () => {
  ...
  describe('Formatter.fromJsonLike', () => {
    ...
    it('method `fromJsonLike(\'{"ali_pay_response":"https:\\/\\/alipay.com","sign":"MA=="}\')` should returns the payload whose keeping the JSON TEXT with escaped dash(/)', () => {
      Formatter.fromJsonLike('{"ali_pay_response":"https:\\/\\/alipay.com","sign":"MA=="}').should.deepEqual({ident: 'ali_pay', payload: 'https:\\/\\/alipay.com', sign: 'MA=='})
    })

    it('method `fromJsonLike(\'{"ali_pay_response"  :  "https:\\/\\/alipay.com"  ,  "sign":"MA=="}\')` should returns the payload whose keeping the JSON TEXT with escaped dash(/) and drop the extra-spaces', () => {
      Formatter.fromJsonLike('{"ali_pay_response"  :  "https:\\/\\/alipay.com"  ,  "sign":"MA=="}').should.deepEqual({ident: 'ali_pay', payload: 'https:\\/\\/alipay.com', sign: 'MA=='})
    })
    ...
    it('method `fromJsonLike(\'{"error_response":{"code":"40004","message":isv.permission=no"},}\')` should deepEqual to {ident: \'error\', payload: \'{"code":"40004","message":isv.permission=no"}\', sign: undefined}', () => {
      Formatter.fromJsonLike('{"error_response":{"code":"40004","message":isv.permission=no"},}').should.deepEqual({ident: 'error', payload: '{"code":"40004","message":isv.permission=no"}', sign: undefined})
    })
  })
