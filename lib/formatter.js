...
class Formatter {
   ...
   */
  static page({baseURL = '', method = 'post', params = {}, data = new URLSearchParams} = {}) {
    ...
    // FIXEDME: While the `charset` wasn't onto the gateway's URL, there shall be a charset-compatible issue.
    //          Debugging the request(`alipay.trade.wap.pay`) workflow as:
    //          pageSubmit(UTF8)-->gateway(UTF8)--302-->mclient(UTF8)--tryToWakeUpAppFailedOrManuallyContinue--200-->tscenter(GBK)--200-->mclient(UTF8)
    //          The problem should be on the `tscenter.alipay.com` which wasnot followed the charset correctly.
    //          Below lines are working with `UTF-8` charset, it shall be optimized in future.
    if (baseURL) {
    ...
    const html = [
      ...
      `</form>`,
      `<script charset="${charset}" type="text/javascript">document.${name}.submit();</script>`,
    ...
    return {
      data: {baseURL, method, params, body, html},
      toJSON() {
        return this.data
      },
      toString() {
